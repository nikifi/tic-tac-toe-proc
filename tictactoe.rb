...
# main game 
def play_game(brd, winning_lines)

  winner = Array.new()

  loop do

      while check_empty(brd).length > 0 && check_winner(brd, winning_lines) == nil

        
        show_board(brd)
        player_turn(brd)

        computer_turn(winning_lines, brd)
      ...
      show_board(brd)
       ...
       if check_winner(brd, winning_lines) != nil
          winner.push(check_winner(brd, winning_lines))
       end
      ...
      elsif score_greater_than_5?(winner, check_winner(brd, winning_lines)) == false
        
        prompt(" #{check_winner(brd, winning_lines)} won this instance!")
        initialize_board(brd)
      
      elsif score_greater_than_5?(winner, check_winner(brd, winning_lines)) == true
  
          prompt(" #{check_winner(brd, winning_lines)} won the game!")  
    ...
    break if score_greater_than_5?(winner, check_winner(brd, winning_lines))
  end
end

#play again

loop do play_game(play_value, winning_lines)
